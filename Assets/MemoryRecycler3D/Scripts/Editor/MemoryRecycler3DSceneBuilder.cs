...
public static class MemoryRecycler3DSceneBuilder
{
    ...
    private static void AddCinematicRoadDetails(Transform root, Material roadPatchMat, Material debrisMat, Material boardMat)
    {
        for (int i = 0; i < 34; i++)
        {
            float z = -30f + i * 2.8f + ReferenceRange(i, 71.3f, -0.65f, 0.65f);
            float x = ReferenceRange(i, 72.9f, -3.2f, 3.2f);
        ...
        for (int i = 0; i < 28; i++)
        {
            ...
            Vector3 position = new Vector3(side * ReferenceRange(i, 77.2f, 5.0f, 8.2f), 0.13f, ReferenceRange(i, 78.1f, -30f, 56f));
        ...
        {
            float side = i % 2 == 0 ? -1f : 1f;
            CreateCinematicWorldBox(root, "Cinematic Fallen Board", new Vector3(side * ReferenceRange(i, 84.4f, 6.0f, 8.6f), 0.17f, ReferenceRange(i, 85.4f, -25f, 52f)), new Vector3(0.24f, 0.10f, 1.65f), boardMat, new Vector3(ReferenceRange(i, 86.4f, -4f, 4f), ReferenceRange(i, 87.4f, -32f, 32f), ReferenceRange(i, 88.4f, -12f, 12f)));
        }
    }
    ...
    private static void EnsureExternalHumanoidWiring(GameObject player, Transform mixamoVisual)
    {
        ...
        playerScript.externalVisualGroundPadding = 0.005f;
    ...
    private static void AddCinematicAvenueDepthCues(Transform root, Material panelMat, Material trimMat, Material cyanMat, Material roadPatchMat)
    {
        ...
        {
            float z = -22f + i * 8.0f;
            float width = Mathf.Lerp(5.6f, 1.5f, i / 9f);
    ...
    private static void AddCinematicArchiveGlow(Transform root, Material cyanMat)
    {
        ...
        Material veilMat = CreateCinematicTransparentMaterial("MR3D_ArchiveGlowVeil", new Color(0.04f, 0.55f, 0.72f, 0.22f), 0.04f);
        Material glassMat = CreateCinematicGlassGlowMaterial();
        CreateCinematicWorldBox(root, "Cinematic Archive Glass Facade", new Vector3(0f, 8.8f, ArchiveCenterZ - 1.88f), new Vector3(1.95f, 17.8f, 0.12f), glassMat, Vector3.zero);
        CreateCinematicWorldBox(root, "Cinematic Archive Glass Spine", new Vector3(0f, 8.8f, ArchiveCenterZ - 2.02f), new Vector3(0.95f, 18.5f, 0.13f), cyanMat, Vector3.zero);
        CreateCinematicWorldBox(root, "Cinematic Archive Glass Core", new Vector3(0f, 8.7f, ArchiveCenterZ - 2.16f), new Vector3(0.46f, 16.8f, 0.15f), cyanMat, Vector3.zero);
        CreateCinematicWorldBox(root, "Cinematic Archive Glass Panel_L", new Vector3(-1.22f, 8.4f, ArchiveCenterZ - 1.72f), new Vector3(0.48f, 15.6f, 0.11f), glassMat, new Vector3(0f, -8f, 0f));
        ...
        for (int i = 0; i < 9; i++)
        {
            ...
            CreateCinematicWorldBox(root, "Cinematic Archive Glass Energy Band", new Vector3(0f, y, ArchiveCenterZ - 2.26f), new Vector3(1.72f, 0.055f, 0.16f), cyanMat, Vector3.zero);
        }
        ...
        panelLight.range = 28f;
        panelLight.intensity = 3.2f;
        panelLight.shadows = LightShadows.None;

        GameObject upperGlassLight = new GameObject("Cinematic Archive Upper Glass Light");
        upperGlassLight.transform.SetParent(root, false);
        upperGlassLight.transform.position = new Vector3(0f, 14.8f, ArchiveCenterZ - 2.0f);
        Light upperLight = upperGlassLight.AddComponent<Light>();
        upperLight.type = LightType.Point;
        upperLight.color = new Color(0.07f, 0.78f, 1.0f);
        upperLight.range = 24f;
    ...
    {
        Color glassColor = new Color(0.08f, 0.88f, 1.0f, 0.72f);
        Material material = CreateCinematicTransparentMaterial("MR3D_ArchiveGlassGlow", glassColor, 0.90f);
        if (material.HasProperty("_EmissionColor"))
            material.SetColor("_EmissionColor", new Color(0.09f, 0.92f, 1.0f) * 5.4f);
        material.EnableKeyword("_EMISSION");
        EditorUtility.SetDirty(material);
        return material;
    }

    private static GameObject CreateCinematicWorldBox(Transform parent, string name, Vector3 position, Vector3 scale, Material material, Vector3 euler, bool local = false, bool keepCollider = false)
    {
        GameObject box = GameObject.CreatePrimitive(PrimitiveType.Cube);
        box.name = name;
    ...
    private static void CreateGround(Material groundMat, Material roadMat, Material debrisMat)
    {
        ...
        for (int i = 0; i < 18; i++)
        {
            ...
            marking.transform.position = new Vector3(0f, 0.05f, -28f + i * 5.0f);
        ...
        for (int i = 0; i < 26; i++)
        {
            ...
            float side = (i % 2 == 0) ? -8.2f : 8.2f;
            rubble.transform.position = new Vector3(side + Random.Range(-2.2f, 2.2f), 0.14f + Random.Range(0f, 0.12f), -30f + i * 3.8f);
